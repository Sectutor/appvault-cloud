...
import os, uuid, json, sqlite3, hmac, hashlib, threading, time, subprocess, urllib.request, base64, re
...
def init_db():
    ...
    if "instance_id" not in lcols:
        db.execute("ALTER TABLE licenses ADD COLUMN instance_id TEXT DEFAULT ''")
    ...
    if "license_key" not in icols:
        db.execute("ALTER TABLE instances ADD COLUMN license_key TEXT DEFAULT ''")
    db.commit()
    db.close()
